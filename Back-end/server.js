...
io.on('connection', (socket) => {
    ...
    socket.on('create-game', (data) => {   // data will send here is gameId
        console.log(`server here listen to event create-game ${data}`);
        socket.join(data);
    });
    ...
    socket.on('join-game', (data) => { // data will send here is gameId
        console.log(`server here listen to event join-game ${data}`);
        ...
        socket.to(data).emit('join', capacity); // send to all clients in room
    });
    ...
    socket.on('start-game', (data) => { // data will send here is gameId
        console.log(`server here listen to event start-game ${data}`);
        socket.broadcast.to(data.gameId).emit('start',{ capacity: data.capacity, turn: data.turn , gameId:data.gameId}); // send to all clients in room except sender
    });
    ...
    socket.on('make-move', (data) => { // data will send here is new position of player and identifier for this player "I don't what it's till now" and gameId
        console.log(`server here listen to event make-move ${data}`);
        socket.broadcast.to(data.gameId).emit('move',{ position: data.position, dice: data.dice , turn: data.turn , nextturn: data.nextturn}); // send to all clients in room except sender
    });
    
    socket.on('end-game',(data)=>{
        console.log(data)
        socket.to(data.gameId).emit('end',"change page")
    })
});
